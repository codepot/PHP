...
<html>
    <head>
        <title><?php echo strtoupper($_SESSION['loginUserFullName']) ?>'S OUTBOX</title>
    ...
    <body>
        ...
        <div id="page-wrap">
            <div class="container">
                <!-- header -->
                <header id="header">
                    <h1 class="title"><?php echo strtoupper($_SESSION['loginUserFullName']) ?>'S OUTBOX</h1>
                </header>
                <!-- Navigation -->
                ...
                <section>
                    ...
                    <div>
                        <table><tr>
                                <th class="th_name"><span>To</span></th>
                    ...
                    <div class="body">
                        <table> <?php
                            if (!isset($view_message)) {
                                ...
                                $query = 'SELECT MessageID, u2.UserFullName, Subject, MsgTime, MsgText, Receiver, p.Status FROM puser u1, puser u2, pmailbox p WHERE u1.Username = Sender AND Sender=\'' . $_SESSION['loginUser'] . '\' AND p.Status != \'deleted by sender\' AND p.Status != \'deleted by both\'  AND u2.Username=Receiver ORDER BY MsgTime DESC';
                                ...
                                for ($i = 0; $i < $count; $i++) {
                                    ...
                                    echo ' <tr> <td class="th_name">' . $fullname . '</td> <td class="th_time">' . $msgTime . '</td> <td class="th_subject"><b><a class="subject_link" href="viewMessage.php?messageId=' . $messageId . '&from=outbox&status=' . $status . '">' . $subject . '</a></td>  <td></td> </tr>';
                                }
