...
<html>
    ...
    <body>
        ...
        $moderator = rawurldecode($_GET['moderator']);
        ...
        if (count($_POST) > 0) {
            ...
            if (!isset($message)) {
                ...
                $query = "INSERT INTO ban (ForumName, Username) VALUES "
                        . "('$forumName','$user');";
        ...
        <form class="upload_form" method='post' enctype='multipart/form-data' action=''>
            <span class="formtitle">BAN AN USER</span>
            ...
            $query = "SELECT * FROM puser WHERE (Status='moderator' OR Status = 'user') AND Username NOT IN (SELECT Username FROM ban where ForumName= BINARY '$forumName')";

            $result = mysql_query($query, $conn);
            $count = mysql_numrows($result);
            if ($count == 0) {
                echo '<a id="userManager" href="manageUsers.php">ADD-AN-USER</a>';
            }
            echo '<BR><select name="user" size="6">';
            for ($i = 0; $i < $count; $i++) {
                $username = mysql_result($result, $i, "Username");
                $userFullName = mysql_result($result, $i, "UserFullName");
                if ($username != $loginUserName AND $username !=$moderator) {
